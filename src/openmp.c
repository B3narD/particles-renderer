...
void openmp_stage2() {
    ...
    for (i = 0; i < openmp_output_image.width * openmp_output_image.height; ++i) {
        ...
        TOTAL_CONTRIBS += openmp_pixel_contribs[i];
    }
...
#pragma omp parallel for schedule(guided, 3)
    for (i = 0; i < openmp_particles_count; ++i) {
        // Compute bounding box [inclusive-inclusive]///
        int x_min = max(0, (int)roundf(openmp_particles[i].location[0] - openmp_particles[i].radius));
        int y_min = max(0, (int)roundf(openmp_particles[i].location[1] - openmp_particles[i].radius));
        int x_max = min(openmp_output_image.width - 1, (int)roundf(openmp_particles[i].location[0] + openmp_particles[i].radius));
        int y_max = min(openmp_output_image.height - 1, (int)roundf(openmp_particles[i].location[1] + openmp_particles[i].radius));
        for (int x = x_min; x <= x_max; ++x) {
            for (int y = y_min; y <= y_max; ++y) {
                const float x_ab = (float)x + 0.5f - openmp_particles[i].location[0];
                const float y_ab = (float)y + 0.5f - openmp_particles[i].location[1];
                ...
                if (pixel_distance <= openmp_particles[i].radius) {
                    ...
                    // Increment openmp_pixel_contribs, so next contributor stores to correct offset
                     //storage_offset;
                    unsigned int storage_offset, tmp;
#pragma omp atomic capture
                    tmp = openmp_pixel_contribs[pixel_offset]++;
                    storage_offset = openmp_pixel_index[pixel_offset] + tmp;
                    // Copy data to openmp_pixel_contrib buffers
                    // critical will drag speed down
